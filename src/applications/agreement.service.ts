...
import { decrypt } from 'src/common/encryption.util';
import { APP_TIME_ZONE } from 'src/common/utils/timezone';
...
// Palette mirrored from the Northstar_Loan_Agreement_Template.pdf.
const COLORS = {
  gold: '#C8911B',
  dark: '#0F172A',
  heading: '#111827',
...
export class AgreementService {
  ...
  // the Northstar_Loan_Agreement_Template.pdf layout (3 pages). When opts.signed
  ...
  private renderPdf(
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        const borrowerFirst = application.first_name || '';
        const borrowerLast = application.last_name || '';
        ...
        const header = () => {
          ...
          // Two-tone wordmark: "Northstar" gold + " Lending" dark.
          doc.fillColor(COLORS.gold).text('Northstar', CONTENT_LEFT, topY, {
            continued: true,
          });
          ...
          doc
            ...
            .text(
              '355 S Grand Ave, Office #20 W, Los Angeles, CA 90071 | support@northstarlend.com',
              CONTENT_LEFT,
              doc.y + 4,
              { width: CONTENT_WIDTH },
            );
        ...
        doc
          ...
          .text(
            `This Personal Loan Agreement (the "Agreement") is entered into as of ${agreementDate}, by and between Northstar Lending ("Lender") and the individual identified below ("Borrower").`,
            CONTENT_LEFT,
            doc.y,
        ...
        const addressLine = [
          application.address,
          application.city,
          `${application.state || ''} ${application.zip_code || ''}`.trim(),
        ]
          .filter(Boolean)
          .join(', ');
        ...
        infoTable([
          ...
          ['Borrower Email', application.email],
        ]);
        ...
        doc.text(
          'Authorized Representative for Northstar Lending.',
          rightX,
          introY,
          { width: colW, lineGap: 2 },
        );
        ...
        doc
          ...
          .fontSize(9.5)
          .fillColor(COLORS.heading)
          .text('Signature: Northstar Lending Admin', rightX, sigLineY + 6, {
            width: colW,
          });
